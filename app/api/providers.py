...
client_schema = Providerschema()
Providers_schema = Providerschema(many=True)

# RUTA
appUrl = '/app/v1/providers'

# INDEX
@app.route(appUrl, methods=['GET'])
def all_Providers():
        all_client = Providers.query.all()
        result = Providers_schema.dump(all_client)
...
def getProviders(id):
        if(request.method == "GET"):
                all_client = Providers.query.get(id)
...
def addProviders():
        if(request.method == "POST"):
                ...
                nuevo_registro=Providers(enterprise_id,type_document,number_doc,business_name,email,phone,phone_local,country,address,status)
...
def updateProviders(id):
        if(request.method == "PUT"):
                actualizar = Providers.query.get(id)
...
def destroyProviders(id):
        if(request.method == "DELETE"):
                eliminar = Providers.query.get(id)
